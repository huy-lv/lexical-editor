...
import {$createNumberInputNode} from '../../nodes/NumberInputNode';
import {$createSingleChoiceInputNode} from '../../nodes/SingleChoiceInputNode';
import {$createMultipleChoiceInputNode} from '../../nodes/MultipleChoiceInputNode';
...
export default function ToolbarPlugin({
  ...
  const exportToHtml = useCallback(() => {
    const htmlString = $generateHtmlFromNodes(activeEditor, null);
    const blob = new Blob([htmlString], {type: 'text/html'});
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'lexical-content.html';
  ...
  return (
    <div className="toolbar">
      ...
      <button
        ...
        type="button"
        className="toolbar-item"
        aria-label="Export content to HTML">
            ...
            <>
              ...
              <DropDown
                ...
                <DropDownItem
                  onClick={() => {
                    activeEditor.update(() => {
                      const numberInputNode = $createNumberInputNode(1);
                      const selection = $getSelection();
                      if ($isRangeSelection(selection)) {
                        selection.insertNodes([numberInputNode]);
                      }
                    });
                  }}
                  className="item">
                  <i className="icon number" />
                  <span className="text">Number Input</span>
                </DropDownItem>
                <DropDownItem
                  onClick={() => {
                    activeEditor.update(() => {
                      const singleChoiceInputNode =
                        $createSingleChoiceInputNode('', [
                          'Option 1',
                          'Option 2',
                        ]);
                      const selection = $getSelection();
                      if ($isRangeSelection(selection)) {
                        selection.insertNodes([singleChoiceInputNode]);
                      }
                    });
                  }}
                  className="item">
                  <i className="icon list" />
                  <span className="text">Single Choice Input</span>
                </DropDownItem>
                <DropDownItem
                  onClick={() => {
                    activeEditor.update(() => {
                      const multipleChoiceInputNode =
                        $createMultipleChoiceInputNode('', [
                          'Option 1',
                          'Option 2',
                        ]);
                      const selection = $getSelection();
                      if ($isRangeSelection(selection)) {
                        selection.insertNodes([multipleChoiceInputNode]);
                      }
                    });
                  }}
                  className="item">
                  <i className="icon list" />
                  <span className="text">Multiple Choice Input</span>
                </DropDownItem>
                <DropDownItem
                  onClick={() => {
